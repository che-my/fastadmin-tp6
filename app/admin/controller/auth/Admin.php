...
class Admin extends Backend
{
    ...
    public function index()
    {
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                    ...
                    ->order($sort, $order)
                    ->count();
            $list = $this->model->where($where)->where('id', 'in', $this->childrenAdminIds)
    ...
    public function add()
    {
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                $this->validate($params,\app\admin\validate\Admin::class,$message); 
    ...
    public function edit($ids = NULL)
    {
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                $this->validate($params,$rule,$message); 
    ...
    public function del($ids = "")
    {
        if ($ids)
        {
            // 避免越权删除管理员
            $childrenGroupIds = $this->childrenGroupIds;
            $uids = Db::name('auth_group_access')->where('group_id', 'in', $childrenGroupIds)->column('uid');
            $adminList = $this->model->where('id', 'in', $ids)->where('id', 'in',$uids)->select();
            if ($adminList)
            {
                $deleteIds = [];
