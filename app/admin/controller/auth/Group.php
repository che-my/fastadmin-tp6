...
class Group extends Backend
{
    ...
    public function initialize()
    {
        ...
        $groupList = $this->model->where('id', 'in', $this->childrenGroupIds)->select()->toArray();
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            $list = AuthGroup::select(array_keys($this->groupdata))->toArray();
            $groupList = [];
            foreach ($list as $k => $v) {
                $groupList[$v['id']] = $v;
            }
    ...
    public function roletree()
    {
        ...
        if (($pid || $parentGroupModel) && (!$id || $currentGroupModel)) {
            $id = $id ? $id : null;
            $ruleList = AuthRule::order(['weigh'=>'desc','id'=>'asc'])->select()->toArray();
            //读取父类角色所有节点列表
            $parentRuleList = [];
            ...
            //角色组列表
            $groupTree->init($this->model->where('id', 'in', $this->childrenGroupIds)->select()->toArray());
