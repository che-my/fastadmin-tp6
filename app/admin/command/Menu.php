...
class Menu extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
            ...
            $list = $this->model->where(function ($query) use ($controller, $equal) {
                foreach ($controller as $index => $item) {
                    if (stripos($item, '_') !== false) {
                        $item = App::parseName($item, 1);
                    }
                    if (stripos($item, '/') !== false) {
                        $controllerArr = explode('/', $item);
                        end($controllerArr);
                        $key = key($controllerArr);
                        $controllerArr[$key] = App::parseName($controllerArr[$key]);
                    } else {
                        $controllerArr = [App::parseName($item)];
                    }
        ...
        if (!in_array('all-controller', $controller)) {
            ...
            //生成权限规则备份文件
            file_put_contents(app()->getRuntimePath() . 'authrule.json', json_encode(collection($authRuleList)->toArray()));
    ...
    protected function importRule($controller)
    {
        ...
        if (isset($matches[1]) && isset($matches[1][0]) && $matches[1][0]) {
            \think\facade\Request::instance()->root('admin');
