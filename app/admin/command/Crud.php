...
class Crud extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $dbconnect = Db::connect(config($db));
        ...
        if ($relation) {
            ...
            foreach ($relationArr as $index => $relationTable) {
                ...
                $relations[] = [
                    ...
                    //关联表外键
                    'relationForeignKey'    => isset($relationForeignKey[$index]) ? $relationForeignKey[$index] : App::parseName($relationName) . '_id',
                    //关联表主键
        ...
        $baseFileName = App::parseName(array_pop($baseNameArr), 0);
        ...
        $viewArr[] = App::parseName($lastValue, 0);
        array_unshift($viewArr, 'view');
        $viewDir = $adminPath . strtolower(implode(DS, $viewArr)) . DS;
        $controllerBaseName = str_replace('_','',$controllerBaseName);
        //最终将生成的文件路径
        ...
        foreach ($relations as $index => &$relation) {
            if ($relation['relationMode'] == 'hasone') {
                ...
                $relationForeignKey = $relation['relationForeignKey'] ? $relation['relationForeignKey'] : App::parseName($relation['relationName']) . "_id";
            ...
            foreach ($columnList as $k => $v) {
                ...
                if ($v['DATA_TYPE'] != 'text' && $inputType != 'fieldlist') {
                    ...
                    if ($this->headingFilterField && $this->headingFilterField == $field && $itemArr) {
                        $headingHtml = $this->getReplacedStub('html/heading-html', ['field' => $field, 'fieldName' => App::parseName($field, 1, false)]);
                    }
            ...
            if ($relations) {
                ...
                foreach ($relations as $index => $relation) {
                    ...
                    if ($relation['relationFields']) {
                        $relationVisibleFieldList[] = "\$row->visible(['".$relation['relationMethod']."'=>['" . implode("','", $relation['relationFields']) . "']]);";
                    }
                }
...
EOD;
    ...
    protected function getParseNameData($module, $name, $table, $type)
    {
        $arr = [];
        if (!$name) {
            $parseName = App::parseName($table, 1);
            $parseArr = [$table];
        } else {
