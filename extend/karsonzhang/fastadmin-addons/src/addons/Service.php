...
use think\Exception;
use ZipArchive;
require app()->getRootPath().'extend/karsonzhang/fastadmin-addons/src/common.php';
/**
 * 插件服务
...
class Service
{
    ...
    public static function download($name, $extend = [])
    {
        ...
        $url = self::getServerUrl() . '/addon/download';
//        p($url);die;
    ...
    {
        $file = app()->getRuntimePath() . 'addons' . DS . $name . '.zip';
    ...
    {
        $file = app()->getRuntimePath() . 'addons' . DS . $name . '-backup-' . date("YmdHis") . '.zip';
        $dir = ADDON_PATH . $name . DS;
        if (class_exists('ZipArchive')) {
            $zip = new ZipArchive;
    ...
    public static function refresh()
    {
        ...
        foreach ($addons as $name => $addon) {
            ...
            if (isset($addon['state']) && $addon['state'] && is_file($bootstrapFile)) {
...
EOD;
        ...
        $file = ROOT_PATH .  DS .'config' . DS . 'addons.php';
