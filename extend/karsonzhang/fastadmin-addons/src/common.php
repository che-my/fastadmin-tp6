...
Event::listen('app_init', function () {
    ...
    $domains = [];
    $rules = [];
    $execute = "\\think\\addons\\addons\\Route@execute?addon=%s&controller=%s&action=%s";
    ...
    // 获取系统配置
    $Events = App::isDebug() ? [] : Cache::get('Events', []);
    if (empty($Events)) {
        $Events = (array)Config::get('addons.Events');
        // 初始化钩子
        foreach ($Events as $key => $values) {
            ...
            $Events[$key] = array_filter(array_map('get_addon_class', $values));
        }
        Cache::set('Events', $Events);
    }
    //如果在插件中有定义app_init，则直接执行
    if (isset($Events['app_init'])) {
        foreach ($Events['app_init'] as $k => $v) {
            Event::exec($v, 'app_init');
        }
    }
    Event::import($Events, false);
});
...
function get_addon_autoload_config($truncate = false)
{
    ...
    foreach ($addons as $name => $addon) {
        ...
        // 跟插件基类方法做比对，得到差异结果
        $Events = array_diff($methods, $base);
        // 循环将钩子方法写入配置中
        foreach ($Events as $Event) {
            $Event = App::parseName($Event, 0, false);
            if (!isset($config['Events'][$Event])) {
                $config['Events'][$Event] = [];
            }
            // 兼容手动配置项
            if (is_string($config['Events'][$Event])) {
                $config['Events'][$Event] = explode(',', $config['Events'][$Event]);
            }
            if (!in_array($name, $config['Events'][$Event])) {
                $config['Events'][$Event][] = $name;
            }
        }
